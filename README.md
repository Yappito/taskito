...
SMTP connections and TLS handshakes default to a 10-second deadline (`SMTP_CONNECT_TIMEOUT_MS`), and each message has a 60-second hard deadline (`SMTP_MESSAGE_TIMEOUT_MS`). Both per-unit timeouts are clamped to at most 300000ms (the scheduler lock safety margin) so a single SMTP send can never hold the scheduler's advisory lock past any sane budget — a configured value above that cap is clamped down with a logged warning. On timeout, Taskito destroys the socket so its bounded email queue can continue with later messages.
...
- Outbound requests are capped by `WEBHOOK_TIMEOUT_MS` (default 10s, clamped 1s–120s) — the same value the claim-lease floor and the pre-POST lease renewal derive from, so under normal clock conditions the renewed lease outlives the request (see the accepted clock-skew residual above). The send-time creator-access re-check is bounded by `WEBHOOK_PREFLIGHT_BUDGET_MS` and fails closed (no POST, bounded retry) when the query stalls past its deadline. HTTP redirects are **never** followed, and response bodies are stream-discarded after 64 KB (the delivery only needs the status code) and never reflected into errors or logs.
...
| `WEBHOOK_LEASE_MARGIN_MS` | `5000` | Buffer over the POST timeout applied when the dispatcher renews the claim lease immediately before the POST (clamped to 2000–60000; a configured value below the 2000ms floor is raised to it, and one above the cap is clamped down, each with a logged warning) |
...
| `SMTP_CONNECT_TIMEOUT_MS` | No | TCP connect, implicit-TLS, and STARTTLS handshake deadline in milliseconds; defaults to `10000`, clamped to at most `300000` (the scheduler lock safety margin) |
| `SMTP_MESSAGE_TIMEOUT_MS` | No | Hard deadline for one full SMTP message conversation in milliseconds; defaults to `60000`, clamped to at most `300000` (the scheduler lock safety margin) |
