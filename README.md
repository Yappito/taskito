...
1. Takes a SESSION-scoped Postgres advisory lock (`pg_try_advisory_lock`, `src/server/services/scheduler.ts`) on a DEDICATED lock connection (its own single-connection PrismaClient, `src/server/services/scheduler-lock-connection.ts`), so in multi-replica deployments only one instance actually runs jobs — the others skip the tick. Because the lock is session-scoped (not bound to a transaction) it cannot be released by transaction expiry while jobs are still live, and because the lock connection is a separate pool (`connection_limit=1`) no job ever has to wait on the connection that holds the lock. The lock is released with `pg_advisory_unlock` on that same connection exactly when the run settles, and the connection is then closed; if the process dies mid-run the session (and the lock) is dropped by Postgres. Durable per-job idempotency (recurrence CAS, digest claims, automation firings, webhook delivery leases) remains the second safety boundary.
