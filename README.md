...
| `SCHEDULER_TICK_TIMEOUT_MS` | `600000` | Maximum duration of one tick's work (jobs are cancelled between units when exceeded; the separate lock-transaction timeout is `SCHEDULER_LOCK_TX_TIMEOUT_MS`) |
| `SCHEDULER_LOCK_TX_TIMEOUT_MS` | `86400000` | Interactive-transaction timeout for the scheduler's pinned advisory-lock transaction (24h) — must stay above the longest possible run so the lock transaction is never expired mid-jobs |
...
- A delivery is claimed **exclusively** (atomic `pending → processing` transition guarded by a lease) so the inline pass and the scheduler sweep can never double-deliver the same event; the lease window is `WEBHOOK_DELIVERY_LEASE_MS` floored at preflight budget + POST timeout (so it can never expire while a real request is in flight), is stamped from the moment THIS row is claimed (not the sweep's batch start), and leases of crashed workers are deliberately recovered back to `pending` on the next sweep.
...
- Outbound requests are capped by `WEBHOOK_TIMEOUT_MS` (default 10s, clamped 1s–120s) — the same value the claim-lease floor is derived from, so the lease always outlives the request. HTTP redirects are **never** followed, and response bodies are stream-discarded after 64 KB (the delivery only needs the status code) and never reflected into errors or logs.
...
| `WEBHOOK_DELIVERY_LEASE_MS` | `300000` | How long a delivery row may stay `processing` (exclusive claim lease) before the scheduler recovers it back to `pending`; floored at preflight budget + POST timeout |
| `WEBHOOK_PREFLIGHT_BUDGET_MS` | `15000` | Wall-time budget for the send-time URL/DNS preflight per attempt (capped so preflight + one POST always fit inside the lease) |
| `WEBHOOK_TIMEOUT_MS` | `10000` | Outbound POST timeout (clamped 1s–120s); single source shared by the request and the lease floor |
